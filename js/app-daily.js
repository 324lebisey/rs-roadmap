...
/* 같은 대분류 안에서 세부 순서만 바꾼다 — 끌던 세부가 놓은 칩의 자리로 들어간다. 이름·예산·납부일·기록은 건드리지 않는다 */
function reorderDlSub(grp,cat,sub,target){
  if(!dailyTree)loadDailyCats();
  if(cat==='대출이자')return;
  var arr=dailyTree[grp]&&dailyTree[grp][cat];if(!arr)return;
  var i=arr.indexOf(sub),j=arr.indexOf(target);
  if(i<0||j<0||i===j)return;
  arr.splice(i,1);arr.splice(j,0,sub);
  dailyCats=dlFlattenCats(dailyTree);
  saveDailyCats();
  renderCatTree();renderActiveView();
}
...
function catGripMove(ev){
  ...
  if(!t&&el&&el.closest){var sc=el.closest('.cat-sub-t');   /* 같은 대분류의 다른 세부 칩만 순서 대상 */
    if(sc&&sc.getAttribute('data-grp')===_catDrag.grp&&sc.getAttribute('data-cat')===_catDrag.cat&&sc.getAttribute('data-sub')!==_catDrag.sub)t=sc;}
...
function catGripUp(ev){
  ...
  if(!d.over)return;
  if(d.over.classList.contains('cat-sub-t')){reorderDlSub(d.grp,d.cat,d.sub,d.over.getAttribute('data-sub'));return;}
...
function renderMoodOptions(){var row=g('dlMoodRow');if(!row)return;if(!dailyMoods)loadDailyMoods();var h='';dailyMoods.forEach(function(m){var on=(m===dailyMood)?' on':'';h+='<span style="position:relative;display:inline-flex">';h+='<button type="button" class="dl-mood'+on+'" data-m="'+dlEsc(m)+'" onclick="selectDailyMood('+jsArg(m)+')">'+dlEsc(m)+'</button>';if(dlMoodManage&&dailyMoods.length>1)h+='<button class="segtip" type="button" onclick="delDailyMood('+jsArg(m)+')" data-tip="삭제" style="position:absolute;top:-7px;right:-7px;width:16px;height:16px;line-height:14px;text-align:center;border-radius:50%;border:none;background:#d9534f;color:#fff;font-size:13px;cursor:pointer;padding:0">\u00d7</button>';h+='</span>';});h+='<button type="button" class="btn btn-ol segtip" onclick="addDailyMood()" style="font-size:13px;padding:5px 9px" data-tip="기분 추가">+</button>';h+='<button type="button" class="btn btn-ol" onclick="toggleMoodManage()" style="font-size:13px;padding:5px 9px">'+(dlMoodManage?'완료':'관리')+'</button>';row.innerHTML=h;updateMoodLabel();}
...
function groupSpendOf(scope,dateStr){var ents=(scope==='weekly')?weekEntries(dateStr):monthEntries(dateStr);var f=0,v=0,sp=0,e2=0;ents.forEach(function(e){var a=entrySpend(e);if(a===0)return;var grp=catGroupOf(e.cat||e.category||'');if(grp==='고정')f+=a;else if(grp==='변동')v+=a;else if(grp==='특별')sp+=a;else e2+=a;});return {fix:f,'var':v,spec:sp,etc:e2};}
/* 합이 정확히 100.0%가 되는 소수 첫째 자리 비율(최대잔여법) */
function pctSplit(a){var t=a.reduce(function(x,y){return x+y;},0);if(t<=0)return a.map(function(){return '0.0';});var r=a.map(function(v){return v/t*1000;}),f=r.map(Math.floor),left=1000-f.reduce(function(x,y){return x+y;},0);r.map(function(v,i){return {i:i,d:v-f[i]};}).sort(function(x,y){return y.d-x.d;}).slice(0,left).forEach(function(o){f[o.i]++;});return f.map(function(n){return (n/10).toFixed(1);});}
function cbKeptSum(ents){return ents.reduce(function(s,e){if(dlCatDeleted(e.cat||e.category||''))return s;var a=parseFloat(e.amount)||0,cb=parseFloat(e.cashback)||0;return s+((a>0&&cb>0&&e.cbDeduct===false)?cb:0);},0);}
function cbKeptRowHtml(cbSum){return cbSum>0?'<div style="display:flex;justify-content:space-between;align-items:baseline;gap:8px;flex-wrap:wrap;margin-top:10px;padding-top:9px;border-top:1px dashed #e4d3d6;font-size:13px"><span style="color:var(--gray);white-space:nowrap">캐시백 <span style="font-size:11px;padding:1px 7px;border-radius:99px;background:#f3eaec;color:var(--ac);margin-left:4px;white-space:nowrap">지출에서 차감 안 함</span></span><span style="font-weight:600;color:var(--ac);white-space:nowrap">'+fmtComma(cbSum)+'원</span></div>':'';}
function budgetCard(title,budget,spent,onch,locked,scope,seg,cbSum){
  ...
  if(seg){var _sf=seg.fix||0,_sv=seg['var']||0,_ss=seg.spec||0,_se=seg.etc||0,_sp=_sf+_sv+_ss+_se;var _dn=budget>0?budget:(_sp>0?_sp:1);var _rf=_sf/_dn*100,_rv=_sv/_dn*100,_rs=_ss/_dn*100,_re=_se/_dn*100,_rt=_rf+_rv+_rs+_re,_sc=_rt>100?100/_rt:1;_rf*=_sc;_rv*=_sc;_rs*=_sc;_re*=_sc;var _pc=pctSplit([_sf,_sv,_ss,_se]),_pf=_pc[0],_pv=_pc[1],_ps=_pc[2],_pe=_pc[3];var _tip='고정 '+fmtComma(_sf)+'원'+(_sp>0?' ('+_pf+'%)':'')+'§§변동 '+fmtComma(_sv)+'원'+(_sp>0?' ('+_pv+'%)':'')+(_ss>0?('§§특별 '+fmtComma(_ss)+'원 ('+_ps+'%)'):'')+(_se>0?('§§기타 '+fmtComma(_se)+'원 ('+_pe+'%)'):'');barInner='<div class="segtip" tabindex="0" data-tip="'+_tip+'" style="margin-bottom:6px"><div style="height:8px;background:#eee;border-radius:99px;overflow:hidden;display:flex">'+(_rf>0?'<div style="height:100%;width:'+_rf+'%;background:'+groupColor('고정')+'"></div>':'')+(_rv>0?'<div style="height:100%;width:'+_rv+'%;background:var(--ac)"></div>':'')+(_rs>0?'<div style="height:100%;width:'+_rs+'%;background:'+groupColor('특별')+'"></div>':'')+(_re>0?'<div style="height:100%;width:'+_re+'%;background:'+groupColor('')+'"></div>':'')+'</div></div>';}
...
var dlCalLayers={payout:1,fixed:1,record:1,income:1,mood:1};
...
function dlCalData(y,m1){
  ...
  var o={dim:dim,days:{},payN:0,payWon:0,fixN:0,fixWon:0,recN:0,recWon:0,incN:0,incWon:0,incUndated:0,carrot:0,whip:0,maxSpent:0};
  /* 수입(세션 81) — 각 수입 항목의 「수입일」(월간 정산 화면에서 항목별 지정, 고정지출 납부일과 같은 effDueDate 규칙)로 칸에 올린다.
     항목이 속한 정산월(K)과 그 날짜의 정산월이 같을 때만 그 달에 나온다. 금액은 항목 그대로(새 계산 없음) — 날짜 없는 항목은 칩에 「날짜 미지정」으로만 센다. */
  var inc={};
  if(dlCalLayers.income){var _ym=y+'-'+String(m1).padStart(2,'0'),_k1=monthKey(_ym+'-01'),_k2=monthKey(_ym+'-'+String(dim).padStart(2,'0'));
    (_k1===_k2?[_k1]:[_k1,_k2]).forEach(function(K){(incomeData[K]||[]).forEach(function(it){var amt=parseInt(it.amount,10)||0;if(amt<=0)return;
      var ef=incomeDayOf(it,K);if(!(ef.day>0)){o.incUndated++;return;}
      var ids=effDueDate(y,m1,ef.day,ef.shift);if(ids.slice(0,7)!==_ym||monthKey(ids)!==K)return;
      var dd=parseInt(ids.slice(8,10),10);(inc[dd]=inc[dd]||[]).push({nm:String(it.name||it.cat||'수입'),won:amt,shifted:(ids!==effDueDate(y,m1,ef.day,false))});o.incN++;o.incWon+=amt;});});}
  for(var d=1;d<=dim;d++){
    var ds=y+'-'+String(m1).padStart(2,'0')+'-'+String(d).padStart(2,'0');
    var c={ds:ds,pay:[],fix:[],spent:0,recs:[],mood:null,inc:(inc[d]||[])};
...
function renderDlCal(){
  ...
  var chips="<div class='dcal-chips'>"
    ...
    +chip('income',"<span class='dcal-sw dcal-inc'></span>",'수입',D.incN?(D.incN+'건 · '+fmtComma(D.incWon)+'원'):(D.incUndated?'날짜 미지정':'없음'))
  ...
  for(var d=1;d<=D.dim;d++){
    ...
    if(c.inc.length)mk+="<span class='dcal-mk dcal-inc'>"+new Array(Math.min(c.inc.length,3)+1).join('●')+"</span>";
  ...
  for(var d2=1;d2<=D.dim;d2++){
    var c2=D.days[d2];
    if(!(c2.pay.length||c2.fix.length||c2.spent||c2.mood||c2.inc.length))continue;
    var items='',sum=0,n=0;
    ...
    if(!fcat)c2.inc.forEach(function(it){items+=dlCalItem('dcal-inc',dlEsc(it.nm)+(it.shifted?" <span class='dcal-sub'>휴일 순연</span>":''),it.won);sum+=it.won;n++;});
...
function offBudgetCalc(scope,dateStr){if(typeof ensureLoanCats==='function')ensureLoanCats();if(!dailyTree)loadDailyCats();var entries=(scope==='weekly')?weekEntries(dateStr):monthEntries(dateStr);var spent={},spentSub={};entries.forEach(function(e){var a=entrySpend(e);if(a===0)return;var c=e.cat||e.category||'미분류';spent[c]=(spent[c]||0)+a;if(e.sub){var sk=subKey(c,e.sub);spentSub[sk]=(spentSub[sk]||0)+a;}});var offByCat={},offTotal=0,_obm=catBudgetMap(scope,dateStr);dailyCats.filter(function(c){return catGroupOf(c)!=='특별';}).forEach(function(c){var ownSet=(parseFloat(_obm[c])||0)>0;var ss=subBudgetSum(scope,c,dateStr);if(ownSet&&ss===0)return;var subsList=(dailyTree[catGroupOf(c)]&&dailyTree[catGroupOf(c)][c])||[];var subMap={},catOff=0,sumSub=0;subsList.forEach(function(sb){var sk=subKey(c,sb);var sa=spentSub[sk]||0;if(sa<=0)return;sumSub+=sa;var subBudgeted=(parseFloat(_obm[sk])||0)>0;if(ss>0){if(!subBudgeted){subMap[sb]=sa;catOff+=sa;}}else{subMap[sb]=sa;catOff+=sa;}});var noSub=(spent[c]||0)-sumSub;if(noSub>0){subMap['(기타)']=(subMap['(기타)']||0)+noSub;catOff+=noSub;}if(catOff>0){offByCat[c]={total:catOff,subs:subMap};offTotal+=catOff;}});var _anyBdg=false;Object.keys(_obm).forEach(function(k){if((parseFloat(_obm[k])||0)>0)_anyBdg=true;});if(!_anyBdg)offTotal=0;return {byCat:offByCat,total:offTotal};}
function goOffBudget(scope){if(!dlOffBudgetOpen){dlOffBudgetOpen=true;renderActiveView();}var el=g('dlOffBudget_'+scope);if(el)el.scrollIntoView({behavior:'smooth',block:'center'});}
function settlementCompareHtml(scope,dateStr,isPast){
  ...
  var _offc=offBudgetCalc(scope,dateStr);if(_offc.total>0){var _offp=offBudgetCalc(scope,prevPeriodDate(scope,dateStr)).total;var _ob={c:'예산 외 지출',n:_offc.total,p:_offp,b:0,bt:'<span onclick="goOffBudget(\''+scope+'\')" style="white-space:nowrap;font-size:12px;cursor:pointer;text-decoration:underline">예산 미설정 '+Object.keys(_offc.byCat).length+'분류 ›</span>'};body+=item(_ob,true,'#fff8e6');}
...
/* 예산 인증 이미지용 식단 — 주간 + 식단표를 켠 경우에만. 메모 줄(type 'memo')만 쓴다: 장보기 금액 줄은 금액 미표시 원칙상 제외.
   그 주 내내 빈 줄(예: 아침)은 열에서 뺀다. 전부 비면 null(섹션 자체를 그리지 않음) */
function certMealData(scope,dateStr){
  if(scope!=='weekly'||!mealOn())return null;
  var wk=mealWkKey(dateStr),dates=mealDatesOf(wk);
  var cols=mealRows(wk).filter(function(r){return r.type==='memo'&&dates.some(function(ds){return String(mealCell(ds,r.id)).trim();});})
    .map(function(r){return {id:r.id,label:r.label||''};});
  if(!cols.length)return null;
  return {cols:cols,days:dates.map(function(ds){return {wd:dlWeekdayKo(ds),cells:cols.map(function(c){return String(mealCell(ds,c.id)).replace(/\s+/g,' ').trim();})};})};
}
function buildBudgetCanvas(scope,dateStr,data){
  ...
  /* 글자 단위로 접는다(어절 하나가 폭보다 넓을 때) — 절대 …로 자르지 않는다 */
  function mealBreak(w,px,tw){mctx.font=px+"px "+FF;var out=[];while(mctx.measureText(w).width>tw&&w.length>1){var k=w.length-1;while(k>1&&mctx.measureText(w.slice(0,k)).width>tw)k--;out.push(w.slice(0,k));w=w.slice(k);}out.push(w);return out;}
  /* 칸 글자는 전부 보여준다: 어절 단위로 접고, 한 어절이 칸보다 넓으면 글자 단위로 이어서 접는다(줄 수·글자 크기 제한 없음) */
  function cellLayout(text,tw){
    if(!text)return {lines:['–'],fs:12,dash:true};
    mctx.font="12px "+FF;
    var lines=[],cur='';
    text.split(' ').forEach(function(w){
      var t=cur?cur+' '+w:w;
      if(mctx.measureText(t).width<=tw){cur=t;return;}
      if(cur){lines.push(cur);cur='';}
      var pc=mealBreak(w,12,tw);cur=pc.pop();pc.forEach(function(x){lines.push(x);});
    });
    lines.push(cur);
    return {lines:lines,fs:12};
  }
  function mealLayout(md){
    if(!md)return null;
    var CW=W-48,n=md.cols.length,h=secHeadH+secGap,out={mode:n<=4?'table':'lines',md:md,rows:[]};
    if(out.mode==='table'){
      var cw=(CW-22)/n,tw=cw-8;
      out.head=md.cols.map(function(c){return mealBreak(c.label,11,tw);});out.cw=cw;out.hl=Math.max.apply(null,out.head.map(function(l){return l.length;}));h+=out.hl*13+7;
      md.days.forEach(function(d){var cells=d.cells.map(function(c){return cellLayout(c,tw);});
        var mx=Math.max.apply(null,cells.map(function(c){return c.lines.length;}));
        out.rows.push({wd:d.wd,cells:cells,h:mx*15+9});h+=mx*15+9;});
    }else{
      /* 열 5개 이상: 요일마다 블록 — 요일 아래에 「라벨 | 내용」 줄(라벨 칸 고정, 내용은 옆에서 접힘). 비어 있는 끼니는 숨기고, 하루 전부 비면 「–」 한 줄 */
      mctx.font="12px "+FF;out.lw=Math.max.apply(null,md.cols.map(function(c){return c.label?mctx.measureText(c.label).width:0;}))+10;
      md.days.forEach(function(d){var items=[];d.cells.forEach(function(c,k){if(c)items.push({label:md.cols[k].label,lines:cellLayout(c,CW-out.lw).lines});});
        var hh=20+6;items.forEach(function(it){hh+=it.lines.length*15+2;});if(!items.length)hh+=15;
        out.rows.push({wd:d.wd,items:items,h:hh});h+=hh;});
    }
    out.h=h;return out;
  }
  ...
  if(meal){
    ...
    if(meal.mode==='table'){
      ctx.fillStyle='#aaa';ctx.font="11px "+FF;
      meal.head.forEach(function(ls,k){ls.forEach(function(t,li){ctx.fillText(t,24+22+k*meal.cw,y+13+li*13);});});
      y+=meal.hl*13+7;
    }
    meal.rows.forEach(function(r,i){
      ctx.strokeStyle='#efe9df';ctx.lineWidth=1;ctx.beginPath();ctx.moveTo(24,y+0.5);ctx.lineTo(W-24,y+0.5);ctx.stroke();
      ctx.fillStyle=dlPalette()[i%dlPalette().length];ctx.font="700 13px "+FF;ctx.textAlign='left';ctx.fillText(r.wd,24,meal.mode==='table'?y+19:y+17);
      if(meal.mode==='table'){
        r.cells.forEach(function(c,k){
          ctx.font=c.fs+"px "+FF;ctx.fillStyle=c.dash?'#ccc':'#333';
          c.lines.forEach(function(l,li){ctx.fillText(l,24+22+k*meal.cw,y+18+li*15);});
        });
      }else{
        var yy=y+35;ctx.font="12px "+FF;
        if(!r.items.length){ctx.fillStyle='#ccc';ctx.fillText('–',24,yy);}
        r.items.forEach(function(it){
          ctx.fillStyle='#aaa';ctx.fillText(it.label,24,yy);
          ctx.fillStyle='#333';it.lines.forEach(function(l,li){ctx.fillText(l,24+meal.lw,yy+li*15);});
          yy+=it.lines.length*15+2;
        });
      }
      y+=r.h;
    });
...
function setCatBudget(scope,cat,v,dateStr){var n=parseFloat(String(v).replace(/,/g,''));n=isNaN(n)||n<0?0:n;var ds=dateStr||todayStr(),map=bWriteMap(scope,ds);if(n>0)map[cat]=n;else delete map[cat];if(scope==='monthly'){var fd=fixedDue[cat];/* 납부일이 정해진 고정지출만 그 납부일이 실제로 걸리는 주에 반영(pruneStaleFixedFromWeek와 짝). 납부일 없는 항목은 월간 예산만 바꾸고 주간은 건드리지 않는다 — 보고 있던 주에 끼어들던 문제 */if(fd&&fd.day){var y=parseInt(ds.slice(0,4),10),mo=parseInt(ds.slice(5,7),10);var weekly=bWriteMap('weekly',effDueDate(y,mo,fd.day,!!fd.shift));if(n>0)weekly[cat]=n;else delete weekly[cat];}}saveDailyBudget();renderActiveView();}
...
/* 고정수입/변동수입(세션 81) — 수입 분류마다 고정 여부. rs_income_fixed 단일 키(분류명 배열, RS_EXTRA_KEYS 등재). 키가 없으면 급여만 고정. 고정수입 행은 다음 달로 자동 이어진다(ensureIncomeSeed). */
var incomeFixed=null;
function loadIncomeFixed(){try{var v=JSON.parse(localStorage.getItem('rs_income_fixed')||'null');incomeFixed=Array.isArray(v)?v.slice():['급여'];}catch(e){incomeFixed=['급여'];}}
function saveIncomeFixed(){try{lsSet('rs_income_fixed',JSON.stringify(incomeFixed));}catch(e){}}
function isIncomeFixed(c){if(!incomeFixed)loadIncomeFixed();return incomeFixed.indexOf(c)>=0;}
function setIncomeFixed(c,on){if(!incomeFixed)loadIncomeFixed();var i=incomeFixed.indexOf(c);if(on&&i<0)incomeFixed.push(c);if(!on&&i>=0)incomeFixed.splice(i,1);saveIncomeFixed();renderIncomeCatList();renderMoIncome(dailyDate||todayStr());}
function _renameIncomeFixed(o,n){if(!incomeFixed)loadIncomeFixed();var i=incomeFixed.indexOf(o);if(i>=0){incomeFixed[i]=n;saveIncomeFixed();}}
function _dropIncomeFixed(c){if(!incomeFixed)loadIncomeFixed();var i=incomeFixed.indexOf(c);if(i>=0){incomeFixed.splice(i,1);saveIncomeFixed();}}
function openIncomeCatModal(){renderIncomeCatList();var m=g('incomeCatModal');if(m)m.classList.add('open');}
function renderIncomeCatList(){var box=g('incomeCatList');if(!box)return;var cats=getIncomeCats();box.innerHTML='<div class="ln-rows">'+cats.map(function(c,i){var fx=isIncomeFixed(c);return '<div class="ln-item" style="padding:9px 0"><input type="text" class="ln-in" value="'+dlEsc(c)+'" onchange="renameIncomeCat('+i+',this.value)" style="flex:1;font-weight:700"><span class="ln-seg segtip" data-tip="고정수입은 다음 달로 자동 이어져요"><button type="button" class="'+(fx?'on':'')+'" onclick="setIncomeFixed('+jsArg(c)+',true)">고정</button><button type="button" class="'+(fx?'':'on')+'" onclick="setIncomeFixed('+jsArg(c)+',false)">변동</button></span><button class="ln-x segtip" type="button" onclick="delIncomeCat('+i+')" data-tip="삭제" style="margin-left:0;'+(cats.length<=1?'opacity:.25;cursor:default':'')+'">\u00d7</button></div>';}).join('')+'</div>';}
...
function renameIncomeCat(i,val){var cats=getIncomeCats();var v=(val||'').trim();if(!v){renderIncomeCatList();return;}if(cats.indexOf(v)>=0&&cats[i]!==v){renderIncomeCatList();return;}var old=cats[i];cats[i]=v;_renameIncomeFixed(old,v);saveIncomeCats();Object.keys(incomeData).forEach(function(mk){(incomeData[mk]||[]).forEach(function(it){if(it.cat===old)it.cat=v;});});saveIncome();applyIncomeToRoadmap(_incMk());renderIncomeCatList();renderMoIncome(dailyDate||todayStr());}
function delIncomeCat(i){var cats=getIncomeCats();if(cats.length<=1)return;_dropIncomeFixed(cats.splice(i,1)[0]);saveIncomeCats();renderIncomeCatList();renderMoIncome(dailyDate||todayStr());}
...
/* 고정수입 자동 이어받기(세션 81) — 이번 정산월에 수입 기록이 아직 없고(키 자체가 없음) 지난달 정산에 「고정수입」 분류 행이 있으면, 그 행(분류·내용·금액·수입일·순연)을 auto:1 표식으로 복사해 둔다.
   ① 이번 정산월에만(과거·미래 달은 건드리지 않는다) ② 키가 이미 있으면(지운 것 포함) 다시 만들지 않는다 ③ 지난달이 손대지 않은 빈 달(키 없음)이면 그 앞달로 건너 찾고, 손댄 달인데 고정수입이 없으면 이어받지 않는다
   ④ 로드맵 연결이 켜져 있으면 기존 write-through(applyIncomeToRoadmap)로 그 달만 반영하고 토스트로 알린다(주인장 확정 — 고정수입은 정해진 날 들어오는 일정한 돈이므로 자동 반영). 부팅 때는 모든 상태 복원이 끝난 뒤(initApp 끝)에만 부른다: 중간에 save()가 돌면 복원 전 부분 상태로 rs7을 덮는다. */
function incomeSeedFor(mk){var k=mk;for(var i=0;i<12;i++){k=prevMonthKey(k);if(incomeData[k]!==undefined){return (incomeData[k]||[]).filter(function(x){return x&&isIncomeFixed(x.cat||'기타');}).map(function(x){var ef=incomeDayOf(x,k);var c={cat:x.cat||'기타',amount:parseInt(x.amount,10)||0,auto:1};if(x.name)c.name=x.name;if(ef.day>0){c.day=ef.day;c.shift=ef.shift;}return c;});}}return [];}
function ensureIncomeSeed(mk){if(!mk||mk!==monthKey(todayStr())||incomeData[mk]!==undefined)return false;var cp=incomeSeedFor(mk);if(!cp.length)return false;incomeData[mk]=cp;saveIncome();if(incomeRoadmapOn){applyIncomeToRoadmap(mk);try{showToast('이번 달 고정수입을 이어받아 로드맵에 반영했어요');}catch(_){}}return true;}
function _incArr(mk){if(!incomeData[mk]&&!ensureIncomeSeed(mk))incomeData[mk]=[];return incomeData[mk];}
...
/* 수입일(세션 81): 항목마다 day(1~31)·shift. 정산월 항목은 매달 새로 만들어지므로, 값이 없으면(undefined) 가까운 지난달의 같은 분류·같은 내용 항목 것을 물려받는다(월간 예산 계승과 같은 발상). day:0은 「날짜 없음」으로 직접 비웠다는 뜻이라 물려받지 않는다. */
function incomeDayOf(it,mk){if(it.day!==undefined)return {day:parseInt(it.day,10)||0,shift:!!it.shift,inh:false};var k=mk;for(var i=0;i<24;i++){k=prevMonthKey(k);var arr=incomeData[k]||[];for(var j=0;j<arr.length;j++){var x=arr[j];if(x.day&&(x.cat||'기타')===(it.cat||'기타')&&String(x.name||'')===String(it.name||''))return {day:parseInt(x.day,10)||0,shift:!!x.shift,inh:true};}}return {day:0,shift:false,inh:false};}
function setIncomeItemDay(idx,val){var mk=_incMk(),a=_incArr(mk);if(!a[idx])return;var ef=incomeDayOf(a[idx],mk);var d=parseInt(val,10);a[idx].day=(d>=1)?Math.min(31,d):0;a[idx].shift=ef.shift;delete a[idx].auto;saveIncome();renderMoIncome(dailyDate||todayStr());}
function setIncomeItemShift(idx,on){var mk=_incMk(),a=_incArr(mk);if(!a[idx])return;var ef=incomeDayOf(a[idx],mk);a[idx].day=ef.day;a[idx].shift=!!on;delete a[idx].auto;saveIncome();renderMoIncome(dailyDate||todayStr());}
...
function setIncomeCat(idx,val){var a=_incArr(_incMk());if(a[idx]){a[idx].cat=val;delete a[idx].auto;saveIncome();applyIncomeToRoadmap(_incMk());renderMoIncome(dailyDate||todayStr());}}
function setIncomeName(idx,val){var a=_incArr(_incMk());if(a[idx]){a[idx].name=String(val||'').trim();delete a[idx].auto;saveIncome();renderMoIncome(dailyDate||todayStr());}}
function setIncomeAmt(idx,val){var a=_incArr(_incMk());if(a[idx]){a[idx].amount=parseInt(String(val).replace(/[^0-9]/g,''),10)||0;delete a[idx].auto;saveIncome();applyIncomeToRoadmap(_incMk());renderMoIncome(dailyDate||todayStr());}}
...
function _incGroupRows(a){var hd=function(t){return '<div class="ln-colh" style="margin-top:10px;padding-bottom:2px">'+t+'</div>';};var h=function(x){return x.h;};var f=a.filter(function(x){return x.fx;}),v=a.filter(function(x){return !x.fx;});return (f.length?hd('고정수입')+f.map(h).join(''):'')+(v.length?hd('변동수입')+v.map(h).join(''):'');}
function moIncomeFlowHtml(dateStr){var dd=dateStr||todayStr();var mk=monthKey(dd);ensureIncomeSeed(mk);
 var items=incomeData[mk]||[];
 ...
 var pct1s=function(v){return (inc>0)?(' · '+(v/inc*100).toFixed(1)+'%'):'';};
 var sumCard='<div style="font-size:13px;letter-spacing:.1em;text-transform:uppercase;color:var(--gray);margin-bottom:9px">'+dlEsc(moLabel)+' 수입 · 순액</div><div style="display:flex;gap:18px;margin-bottom:14px">'
  +'<div style="flex:1;min-width:0"><div style="height:3px;background:var(--carrot-mk,#3f9a68);margin-bottom:8px"></div><div style="font-size:12px;color:var(--gray);white-space:nowrap">수입</div><div style="font-family:var(--serif);font-size:20px;font-weight:600;white-space:nowrap;color:var(--carrot-mk,#3f9a68)">'+fmtComma(inc)+'</div></div>'
  +'<div style="flex:1;min-width:0"><div style="height:3px;background:var(--whip-mk,#d9534f);margin-bottom:8px"></div><div style="font-size:12px;color:var(--gray);white-space:nowrap">지출'+pct1s(exp)+'</div><div style="font-family:var(--serif);font-size:20px;font-weight:600;white-space:nowrap;color:var(--whip-mk,#d9534f)">'+fmtComma(exp)+'</div></div>'
  +'<div style="flex:1;min-width:0"><div style="height:3px;background:'+ncol+';margin-bottom:8px"></div><div style="font-size:12px;color:var(--gray);white-space:nowrap">순액'+pct1s(net)+'</div><div style="font-family:var(--serif);font-size:20px;font-weight:600;white-space:nowrap;color:'+ncol+'">'+(net<0?'−':'')+fmtComma(Math.abs(net))+'</div></div>'
  +'</div>';
 ...
 var _rowH=items.map(function(it,idx){
   ...
   var ef=incomeDayOf(it,mk);
   return {fx:isIncomeFixed(it.cat||'기타'),h:'<div class="ln-item wrap" style="padding:7px 0;border-bottom:1px solid var(--ln-line)">'+(it.auto?'<span class="segtip" data-tip="지난달 급여를 이어서 넣었어요. 금액이 다르면 고쳐 주세요" style="font-size:12px;color:var(--ac);white-space:nowrap">자동</span>':'')+'<select class="ln-in" onchange="setIncomeCat('+idx+',this.value)" style="font-weight:700">'+opts+'</select><input type="text" value="'+dlEsc(it.name||'').replace(/\"/g,'&quot;')+'" placeholder="내용 (선택)" onchange="setIncomeName('+idx+',this.value)" class="ln-in" style="flex:1.3;min-width:90px"><input type="text" inputmode="numeric" value="'+(av?av.toLocaleString():'')+'" placeholder="금액" oninput="commaInput(this)" onchange="setIncomeAmt('+idx+',this.value)" class="ln-in" style="flex:1;min-width:84px;text-align:right"><span style="font-size:13px;color:var(--gray);white-space:nowrap">원</span><span style="display:inline-flex;align-items:center;gap:4px;white-space:nowrap"><input type="number" min="1" max="31" value="'+(it.day!==undefined?(it.day||''):'')+'" placeholder="'+(ef.day?ef.day:'일')+'" onchange="setIncomeItemDay('+idx+',this.value)" class="ln-in" style="width:40px;text-align:center"><span style="font-size:13px;color:var(--gray)">일</span><label class="segtip" data-tip="주말·공휴일이면 다음 영업일" style="font-size:13px;color:var(--gray);display:inline-flex;align-items:center;gap:3px;cursor:pointer;white-space:nowrap"><input type="checkbox" '+(ef.shift?'checked':'')+' onchange="setIncomeItemShift('+idx+',this.checked)" style="margin:0">순연</label></span><button class="ln-x segtip" type="button" onclick="delIncomeItem('+idx+')" data-tip="삭제" style="margin-left:0">×</button></div>'};
 });
 var rows=_incGroupRows(_rowH);
 ...
 var section='<div style="display:flex;justify-content:space-between;align-items:baseline;margin:2px 0 9px"><span style="font-weight:700;font-size:13px"><img src="'+INCOME_IMG+'" alt="수입" style="height:1.15em;vertical-align:-2px"> 이 달 수입'+helpIcon('같은 분류로 여러 줄 넣어도 돼요(예: 계좌별 이자를 금융소득으로 각각). 합계에 자동으로 더해지고, 한 분류가 2건 이상이면 목록 아래에 분류별 소계가 떠요.')+'</span><span style="font-size:13px;color:var(--gray);white-space:nowrap">합계 '+fmtComma(inc)+'원</span></div>'
   +rows
   +'<div style="display:flex;gap:10px;align-items:center;margin-top:2px;flex-wrap:wrap"><button type="button" onclick="addIncomeItem()" style="border:none;background:none;color:var(--ac);font-size:13px;font-weight:700;cursor:pointer;padding:5px 2px;white-space:nowrap">+ 수입 추가</button><button type="button" onclick="openIncomeCatModal()" style="border:none;background:none;color:var(--ac);font-size:13px;cursor:pointer;padding:5px 2px;white-space:nowrap">✏️ 카테고리 편집</button><button type="button" onclick="openIncomeMapModal()" style="border:none;background:none;color:var(--ac);font-size:13px;cursor:pointer;padding:5px 2px;white-space:nowrap">🔗 로드맵 연결</button></div>';
 return '<div class="ln" style="background:#fff;border:1px solid var(--tbl-border);border-radius:10px;padding:13px 13px 15px">'+sumCard+wf+section+_incomeCompareHtml(dd)+'</div>';
}
