...
function settlementCompareHtml(scope,dateStr,isPast){
  ...
  [['고정','🔒 고정비','var(--st-fix-h)','var(--st-fix-y)'],['변동','🔄 변동비','var(--st-var-h)','var(--st-var-y)'],['특별','🎁 특별비','var(--st-spec-h)','var(--st-spec-y)'],['기타','기타','rgba(0,0,0,.045)','rgba(0,0,0,.03)']].forEach(function(g){var set=groups[g[0]];if(!set.length)return;count++;set.sort(function(a,b){return b.n-a.n;});body+='<tr style="background:'+g[2]+'"><td class="st-first" colspan="5" style="padding:7px 6px;font-size:13px;font-weight:700">'+g[1]+'</td></tr>'+set.map(function(x){return item(x,false,'');}).join('');var sum={c:g[0]+'비 계',n:0,p:0,b:0};set.forEach(function(x){sum.n+=x.n;sum.p+=x.p;sum.b+=x.b;all.n+=x.n;all.p+=x.p;all.b+=x.b;});body+=item(sum,true,g[3]);});
  if(count>1)body+=item(all,true,'var(--st-tot)');
  return '<div style="margin:22px 0 16px"><div style="font-size:13px;letter-spacing:.1em;text-transform:uppercase;color:var(--gray);margin-bottom:8px">정산 비교</div><div class="tbl-wrap st-table-wrap"><table class="st-table"><thead><tr style="color:var(--gray)"><th class="st-first" style="text-align:left;padding:4px 6px;font-size:13px">분류</th><th style="text-align:right;padding:4px 6px;font-size:13px">'+current+'</th><th style="text-align:right;padding:4px 6px;font-size:13px">예산대비</th><th style="text-align:right;padding:4px 6px 4px 18px;font-size:13px">'+previous+'</th><th style="text-align:right;padding:4px 6px;font-size:13px">증감</th></tr></thead><tbody>'+body+'</tbody></table></div></div>';
}
