...
function compactMobileSubSummaries(){if(!(window.matchMedia('(max-width:900px)').matches||window.matchMedia('(hover:none)').matches))return;['dlWeekCat','dlMoCat'].forEach(function(id){var root=g(id);if(!root)return;root.querySelectorAll('div').forEach(function(el){if(el.children.length&&Array.prototype.some.call(el.children,function(c){return c.tagName==='DIV';}))return;var text=el.textContent||'';if(text.indexOf('썼어요')<0)return;var spent=(text.match(/썼어요\s*([\d,]+)/)||[])[1];if(!spent)return;var left=(text.match(/남음\s*([\d,]+)원/)||[])[1];var over=(text.match(/([\d,]+)원\s*초과/)||[])[1];var value=left?(parseInt(spent.replace(/,/g,''),10)+' / '+(parseInt(spent.replace(/,/g,''),10)+parseInt(left.replace(/,/g,''),10)).toLocaleString('ko-KR')+'원'):over?(parseInt(spent.replace(/,/g,''),10)+' / '+(parseInt(spent.replace(/,/g,''),10)-parseInt(over.replace(/,/g,''),10)).toLocaleString('ko-KR')+'원'):spent+'원';el.textContent=value;el.classList.add('sub-budget-mobile-summary');});});}
var renderActiveViewBase=renderActiveView;renderActiveView=function(){renderActiveViewBase();compactMobileSubSummaries();};
var renderWeekDueBase=renderWeekDue;
renderWeekDue=function(){renderWeekDueBase();var box=g('dlWeekDue');if(!box||!box.firstElementChild)return;var rows=box.firstElementChild.children;for(var i=1;i<rows.length;i++){var actions=rows[i].lastElementChild,status=actions&&actions.firstElementChild;if(!status)continue;var paid=status.textContent.indexOf('완료')>=0;status.className='week-due-status '+(paid?'week-due-paid':'week-due-pending');status.innerHTML='<span class="week-due-status-icon">'+(paid?'✅':'□')+'</span><span class="week-due-status-text">'+(paid?' 납부 완료':' 납부 전')+'</span>';}}
var renderWeekDueCompactBase=renderWeekDue;
renderWeekDue=function(){renderWeekDueCompactBase();if(!(window.matchMedia('(max-width:900px)').matches||window.matchMedia('(hover:none)').matches))return;var box=g('dlWeekDue');if(!box||!box.firstElementChild)return;var rows=box.firstElementChild.children;for(var i=1;i<rows.length;i++){var main=rows[i].firstElementChild;if(!main||main.classList.contains('week-due-main'))continue;var label=main.children[0],amount=main.children[1],name='';for(var n=0;n<main.childNodes.length;n++){if(main.childNodes[n].nodeType===3)name+=main.childNodes[n].nodeValue;}main.textContent='';main.className='week-due-main';var title=document.createElement('span');title.className='week-due-name';title.textContent=name.trim();var meta=document.createElement('span');meta.className='week-due-meta';if(label)meta.appendChild(label);if(amount)meta.appendChild(amount);main.appendChild(title);main.appendChild(meta);rows[i].classList.add('week-due-row');rows[i].lastElementChild.classList.add('week-due-actions');}}
